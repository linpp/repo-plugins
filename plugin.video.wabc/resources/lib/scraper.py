...
class myAddon(t1mAddon):
  ...
  def getAddonMenu(self,url,ilist):
      urls = {}
      html = ''
      try:  # load local shows override if available
        url = xbmc.translatePath('special://home/addons/plugin.video.wabc/resources/lib/shows.html')
...
#        html = d.decompress(html)
            ...
            if isinstance(tiles,list):
                for show in tiles:
                    ...
                    if urls.get(url,None) is None and show.get('show') is not None:
                        ...
                        contextMenu = [('Add To Library','RunPlugin(%s?mode=DF&url=AL%s)' % (sys.argv[0], url))]
  ...
  def getAddonEpisodes(self,url,ilist, getFileData = False):
      ...
      if len(html) < 150000 or re.search('module-404-wrapper', html, re.DOTALL):    #no root episode-guide
          u = 'https://abc.com'+url
          html = self.getRequest(u)
      ...
      if len(ilist) == 0:
          xbmc.executebuiltin('Notification("%s", "%s", %s, "%s")' % (self.addonName, self.addon.getLocalizedString(30001), 5000, self.addonIcon))
      return(ilist)
  ...
  def getAddonVideo(self,url):
      ...
      # usually there is only one asset, but sometimes there are multiple assets and the first is not playable media
      for u in assets:
        url = u['value']+'?'+sessionKey
        html = self.getRequest(url)
        media = re.compile('#UPLYNK-MEDIA0.+?http(.+?)\n',re.DOTALL).search(html)
        if media is not None:       # found playable media
            url = media.group(1)
            break
      url = 'http'+url
